...
import { GetLeaderDivisionApi } from "../api/leaderDivision.api";
...
export function useGetLeaderDivision(options?: UseGetLeaderDivisionOptions) {
  ...
  const fetchSpp = useCallback(async (query?: QuerySppDTO) => {
    try {
      ...
      const res = await GetLeaderDivisionApi(query);
