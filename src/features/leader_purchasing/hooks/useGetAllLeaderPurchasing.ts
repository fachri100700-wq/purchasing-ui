...
import { GetAllLeaderPurchasingApi } from "../api/leaderPurchasing.api";

interface UseGetLeaderDivisionOptions {
  initialQuery?: QuerySppDTO;
}

export function useGetAllLeaderPurchasing(options?: UseGetLeaderDivisionOptions) {
  ...
  const fetchSpp = useCallback(async (query?: QuerySppDTO) => {
    try {
      ...
      const res = await GetAllLeaderPurchasingApi(query);
