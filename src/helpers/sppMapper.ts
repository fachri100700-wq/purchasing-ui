import type { SppStatus } from "../types/ApiResponse";
...
export const getStatusBadge = (status: SppStatus) => {
  switch (status) {
    ...
    case SPP_STATUS.GUDANG_APPROVED:
      return { text: "Menunggu Approval", colorClass: "bg-sky-100 text-sky-700 border-sky-200" };
    ...
    case SPP_STATUS.NEED_DIRECTOR_APPROVAL:
      return { text: "Sedang Diproses", colorClass: "bg-amber-100 text-amber-700 border-amber-200" };
    case SPP_STATUS.DIRECTOR_APPROVED:
    case SPP_STATUS.PO_CREATED:
    case SPP_STATUS.COMPLETED:
      return { text: "Selesai", colorClass: "bg-emerald-100 text-emerald-700 border-emerald-200" };
    case SPP_STATUS.REJECTED:
      return { text: "Ditolak", colorClass: "bg-rose-100 text-rose-700 border-rose-200" };
    default:
      return { text: "Diproses", colorClass: "bg-slate-100 text-slate-700 border-slate-200" };
  }
};

export const getStepInfo = (status: SppStatus): { step: string; progress: number } => {
  switch (status) {
    case SPP_STATUS.SUBMITTED:
